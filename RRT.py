# -*- coding: utf-8 -*-
"""
Created on Sun Nov 20 17:34:53 2022
...
class RRT:
    ...
    class Node:
        def __init__(self, x, y):
            # 包含坐标(x,y)，每一个节点延申路径集合【(path_x,path_y)】以及父节点parent
            self.x = x
            self.y = y
            self.path_x = []
            self.path_y = []
    ...
    def __init__(self,
        ...
        self.min_rand = rand_area[0]  # 随机取样的区域最小值
        self.max_rand = rand_area[1]  # 随机取样的区域最大值
    ...
    def steer(self, from_node, to_node, extend_length=float("inf")):
        ...
        d, theta = self.calc_distance_and_angle(new_node, to_node)  # 转换为极坐标系，计算两个点之间的距离和角度

        new_node.path_x = [new_node.x]  # 延申路径的起点
        new_node.path_y = [new_node.y]

        if extend_length > d:
            extend_length = d

        n_expand = math.floor(extend_length / self.path_resolution)  # 把这个路径分成n块 path_resolution=0.1

        for _ in range(n_expand):
            new_node.x += self.path_resolution * math.cos(theta)
            new_node.y += self.path_resolution * math.sin(theta)
            new_node.path_x.append(new_node.x)  # 记录每一个衍生节点的坐标
            new_node.path_y.append(new_node.y)

        d, _ = self.calc_distance_and_angle(new_node, to_node) # 找到最后一个延申点和目标点之间的距离
        if d <= self.path_resolution: # 如果距离已经小于分解距离，就把目标点设置为最后一个点
    ...
    def plot_circle(x, y, size, color="-k"):  # pragma: no cover
        ...
        # plt.plot(x, y, "ok", ms=30 * size)

    @staticmethod
    def get_nearest_node_index(node_list, rnd_node):
        # 创建一个距离list，然后找到最小值索引
